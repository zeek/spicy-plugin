...
bool FileAnalyzer::Process(int len, const u_char* data) {
    if ( ! _state.hasParser() && ! _state.isSkipping() ) {
        auto parser = OurPlugin->parserForFileAnalyzer(_state.file().analyzer->Tag());
        if ( parser )
    ...
    if ( _state.file().depth >= max_file_depth ) {
        ...
        file->FileEvent(Spicy::max_file_depth_exceeded,
                        {file_val, analyzer_args, ::zeek::val_mgr->Count(_state.file().depth)});

        auto tag = OurPlugin->tagForFileAnalyzer(_state.file().analyzer->Tag());
    ...
    try {
        ...
        reporter::analyzerError(_state.file().analyzer, e.description(),
